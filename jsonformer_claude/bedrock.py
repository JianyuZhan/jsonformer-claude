...
class JsonformerClaudeBedrock:
    ...
    def __init__(
        ...
        **claude_args,
        ...
        self.debug_on = debug
        self.bedrock_client = boto3_client
    ...
    async def _completion(self, prompt: str):
        ...
        if stream:
            ...
            for response in stream:
              ...
              if chunk:
                  # chunk_response is in the form of : 
                  # {'completion': ' This is a ', 'stop_reason': None, 'stop': None}
                  # {'completion': ' demo stream test.', 'stop_reason': 'stop_sequence', 'stop': '\n\nHuman:', 'amazon-bedrock-invocationMetrics': {'inputTokenCount': 19, 'outputTokenCount': 199, 'invocationLatency': 7820, 'firstByteLatency': 410}}
                  chunk_response = json.loads(chunk.get("bytes").decode())

                  # The chunk_response["completion"] is NOT accumulating itself, so we do cumulation ourselves.
                  self.last_anthropic_response = prompt + cumulated_response + chunk_response["completion"]
                  ...
                  cumulated_response += chunk_response["completion"]
